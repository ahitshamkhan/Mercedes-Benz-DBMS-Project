      ...
      <main className="pt-32 pb-40">
        ...
        <section className="mb-section-gap overflow-hidden">
          <div className="flex gap-8 px-6 md:px-20 overflow-x-auto hide-scrollbar snap-x snap-mandatory">
            {FEATURED.map(f => (
              <Link key={f.id} to={`/car/${f.id}`} className="flex-none w-[80vw] md:w-[60vw] aspect-[21/9] snap-center relative overflow-hidden group gold-hairline bg-surface-container-lowest">
                ...
                <div className="absolute bottom-10 left-10 text-white">
                  <span className="text-primary font-label-sm uppercase tracking-widest mb-2 block">{f.label}</span>
                  <h3 className="font-headline-h2 text-3xl">{f.name}</h3>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="max-w-container-max mx-auto">
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                {cars.map(car => (
                  <div key={car.id} className="bg-[#111118] gold-hairline p-1 group gold-glow transition-all duration-500">
                    <Link to={`/car/${car.id}`}>
                      ...
                      <div className="px-6 pb-8">
                        <p className="text-outline font-label-sm uppercase mb-1">{car.cat}</p>
